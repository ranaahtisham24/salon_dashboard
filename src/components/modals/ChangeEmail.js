...
const ChangeEmail = () => {
    ...
    const { user, loading, isModalOpen } = useSelector(state => state.Auth)
    ...
    const otpRef = useRef()
    // console.log("🚀 ~ file: ChangeEmail.js ~ line 27 ~ ChangeEmail ~ otpRef", otpRef.current.value)
    ...
    const submitHandler = async () => {
        ...
        if (isEmailSent) {
            if (newEmailRef.current.value && passwordRef.current.value && otpRef.current.value) {
                const data = {
